...
| [`typing-escape-hatches/`](typing-escape-hatches/brief.md) — type-checker escape hatches in `src/` | **Commissioned 2026-09-11** at `8e88e4f`. **Inventory delivered 2026-09-17** at `94468bd0` ([`SUMMARY.md`](typing-escape-hatches/SUMMARY.md), [`inventory.md`](typing-escape-hatches/inventory.md)). 89 sites, each probed on both checkers. S1 (`CONTRIBUTING.md` suppression form) and S6 (the twelve pyrefly warnings) closed in the inventory PR. **Q1 decided A (2026-09-18):** public `extra` / `replace` kwargs are `dict[str, object]` / `object`. **TypedDict later (2026-09-20):** the key → type map moves into a PEP 728 `TypedDict`, staged as PR 8 and implemented in this change. S2/S3 still closed; a third `# pyrefly: ignore[bad-override]` (`FullCountStream.name`) is the same S5b KEEP. Next: staged fix PRs 1, 2, 2b, 3, 4, 5, 7 (dead casts, remaining `Any`→`object`, `_raw` after PR 1, `_track_source_seeks` overloads, TypeGuard predicates, remaining Protocols, KEEP comments). |
