...
| [`typing-escape-hatches/`](typing-escape-hatches/brief.md) — type-checker escape hatches in `src/` | **Commissioned 2026-09-11** at `8e88e4f`. Audits every construct that tells Pyrefly/ty to stop looking: 2 `# type: ignore` (both verified dead), 26 `cast(`, 37 `Any`, 3 `TypeGuard`, 13 `assert isinstance`. Four dispositions per site, fixing the type model preferred over re-labelling. Two findings are already fixed: `# type: ignore[code]` is a *blanket* line suppression under pyrefly (the bracketed code is not validated), so `CONTRIBUTING.md:216-217` is wrong and is in scope; and the two suppressions in `src/` are unnecessary. Two more sites live only on PR #324 (finding F6) — sequence after it merges. Deliverable is an **inventory first**, then staged fix PRs by category. |
