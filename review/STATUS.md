# In-flight review status (2026-09-25)

## In flight

| Review | State |
|---|---|
| [`problem-catalogue/`](problem-catalogue/brief.md) — the **problem catalogue** (Topic 10) | **Commissioned 2026-08-15** at `d4668c3`. Not a review: extraction and normalization over ~180 documents that already state a problem (72 change proposals, 57 `design.md`, 17 ADRs, 11 review summaries, 8 investigations, threat model, `known-issues`, `library-analysis`, `dev-docs/history/`). One entry per problem, N sources, stated **solution-neutrally** — the decision that resolved it is a separate strippable field, because the second consumer is a fresh-design comparison run against the problems alone. Parallel with Topic 8 on disjoint sources; takes the code-comment residue from its capability workers. **Delivered 2026-08-17** at `5d08f31` ([`SUMMARY.md`](problem-catalogue/SUMMARY.md)): 146 entries from 489 source attributions. Open only for its §Outstanding: the code-comment harvest (`harvest/` is still a README), two PPMd primary documents read through `known-issues.md`, and the fresh-design experiment, which has not been run. |
| [`docs-content/`](docs-content/brief.md) — documentation **content** (Topic 8) | **Commissioned 2026-08-15** at `d4668c3`, once the library churn the prose was waiting on had landed (`#225`, `#232`, `#233`–`#236`). Five passes, run in sequence rather than triaged — **scope** (D-f: state the contract, do not prove it) → accuracy vs the code → gaps → register (O-16/O-17) → quality — with no budget or target date; the brief's §Definition of done says what completion means. Starts from `docs/observations.md` and `docs/outline.md`. **Pass 0 done 2026-08-17** ([`docs-content/scope.md`](docs-content/scope.md), PR #242): all 16 pages have a stated job, an explicit non-coverage list and every block routed; the outline's "~455 lines outstanding" is **re-tallied to ~174**, with two rows dissolved as already shipped, three converted to docstring or threat-model tasks, and four added that the outline never carried. Its six scope questions are all decided, including a carve-out letting a docs PR carry docstring-only `src/` edits. **Steps 2–3 and pass 1 done 2026-08-18**: [`docs-content/claims.md`](docs-content/claims.md) holds every checkable claim, each with a `verified` / `wrong` / `unverifiable` verdict. **Next: passes 2–4** (gaps, register, quality) and the ~174 lines of prose, tracked internally; `how-it-works.md` is still unwritten. Writes prose directly — a library defect found becomes a separate fix PR, never an edit inside a docs PR. |
...
| [`typing-escape-hatches/`](typing-escape-hatches/brief.md) — type-checker escape hatches in `src/` | **Commissioned 2026-09-11** at `8e88e4f`. **Inventory delivered 2026-09-17** at `94468bd0` ([`SUMMARY.md`](typing-escape-hatches/SUMMARY.md), [`inventory.md`](typing-escape-hatches/inventory.md)). 89 sites, each probed on both checkers. S1 (`CONTRIBUTING.md` suppression form) and S6 (the twelve pyrefly warnings) closed in the inventory PR. **Q1 decided A (2026-09-18):** public `extra` / `replace` kwargs are `dict[str, object]` / `object`. **TypedDict later (2026-09-20, revised same day on #384 K1):** the key → type map is two overloaded `dict[str, object]` subclasses (`MemberExtra`, `ArchiveInfoExtra`), not PEP 728. S2/S3 still closed. **Staged fix PRs done or moot on `main`** (#378, #384, #419, #451, #453): checked 2026-09-25, none of the five dead casts remains, and the two `Any` annotations left in `src/` are the documented keeps A41 (`SeekPoint.state`) and A46 (`BinaryIOWrapper.__init__`). Nothing staged remains; the row stays only until the directory moves to `archive/`, and `SUMMARY.md` items 1 and 2 still need striking through first, since they landed without being marked. |
| [`api-freeze/`](api-freeze/brief.md) — the **public API at the `0.2.0` freeze** | **Commissioned 2026-09-21** at `b0fe664`. A re-review of [`archive/2026-07-19-api-coherence/`](archive/2026-07-19-api-coherence/), whose Q1–Q7 were all decided and implemented — every one verified still true on `main` while the brief was written, and listed in its §Do not re-raise. The case for running it again: `__all__` is 89 against July's 90, but **15 names left and 14 arrived**, and the nine public modules are **+1562/−293** lines against baseline `7139c13` (`core.py` +513, `diagnostics.py` +388, `types.py` +346). No review has judged the surface as a whole since those names landed. The brief's sharpest seed is that the real surface is **120 importable names**, not 89: `__all__` 89, the `# noqa: F401` block 17, plus two top-level modules the package docstring never lists — `detection_cost` (11 names) and `escaping` (3; since #448 `terminal`, documented). Scoped **non-security** on purpose: it is being run on Fable credit, hostile-input framing downgrades the model, and threat analysis hands off to `dev-docs/threat-model.md`. |
| [`exception-catchalls/`](exception-catchalls/brief.md) — blind `except` in `src/` | **Commissioned 2026-09-11** at `8e88e4f`. 55 blind handlers (25 `except Exception`, 30 `except BaseException`, 0 bare `except:`), 30 carrying `# noqa: BLE001`. Measured against the error contract (`CONTRIBUTING.md`, ADR 0012, `error-handling` spec). Scoped as **verification, not excavation**: every marked site already has an inline reason and they fall into five patterns, so a large "what is actually fine" section is the expected outcome. Sharpest seeds: the ~25 *unmarked* handlers (ruff exempts re-raisers — verify that claim) and `verify.py:259`, which substitutes `b""` for an unknown failure on a verification path. Every fix here changes runtime behaviour, so each needs a test. **Done 2026-09-25** at `5bbbfdc` ([`SUMMARY.md`](exception-catchalls/SUMMARY.md)): 69 handlers (the census had grown), 62 fine, seven fixed (a bzip2 accelerator path with no C-boundary trap aborted the process; three unmarked handlers converted every exception to one type). Six were fixed in one PR; the last four handlers (two deferred behind an open PR, two it added) were judged on `c599fc5` after it merged, and one of them was the seventh fix. |
...
lines of new prose the outline identifies and left ~455 for Topic 8. *(State as of that
change. Several of the gaps below closed in `#225`/`#232`/`#235`, and Topic 8's pass 0
re-tallied the remainder to ~174 — see the `docs-content/` row above and
[`docs-content/scope.md`](docs-content/scope.md) §B.)* `install.md` has
...
3. **Topic 10 — the problem catalogue** (`problem-catalogue/`, delivered 2026-08-17 apart
   from its §Outstanding). Topic 8's capability workers still owe it the code-comment
   residue. It **may enrich**
