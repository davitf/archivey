# In-flight review status (2026-09-21)
...
The two reviews commissioned 2026-09-11 (`typing-escape-hatches/`, `exception-catchalls/`) are **not ranked into the list above** — their ordering against items 1–5 is an open maintainer decision. Both are small, bounded, and run in parallel with each other on disjoint sources. The argument for taking them early is freeze proximity: a public signature tightened after `0.2.0`, or a change in which exceptions escape the public API, is a compatibility event, and both reviews exist to find exactly those.
