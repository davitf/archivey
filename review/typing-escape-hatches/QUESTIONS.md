...
### TypedDict later? ✅ **DECIDED 2026-09-20: overloaded dict subclass (not PEP 728)**

**Maintainer (2026-09-20):** make `extra` carry the correct type for every known
key, while still allowing unknown keys for third-party extensions — on its own
PR, not this one.

**Same day, #384 K1:** drop PEP 728. mypy does not support `extra_items` at all
(the TypedDict then has zero keys, every read and write in a user's file
errors). pyright honours `extra_items` but flags a subscript read of a
`total=False` key. The public field is a `dict[str, object]` subclass whose
`__getitem__` is overloaded once per known key, with a `str → object` fallback.
Measured clean on pyright 1.1.414, mypy 1.19.1, pyrefly 1.1.1 and ty 0.0.60.
No `typing_extensions`. The three costs PR 8 carries are listed once, on
[`SUMMARY.md`](SUMMARY.md) item 8 — do not keep a second copy here.

**The contract is now the two classes** (`MemberExtra` / `ArchiveInfoExtra`;
the `EXTRA_*` constants remain the names). Unknown keys stay legal and read as
`object`.
...
A single map that lists every key still makes
...
SUMMARY.md item 8. Per-format aliases (`RarMemberExtra`, …) remain possible
...
already-constructed member; new keys show up with format work. The overloaded
class's `str → object` fallback is what keeps unknown keys legal. **Do not
reintroduce PEP 728 `extra_items`:** it is not in any CPython release, mypy
does not implement it, and the 3.11 floor was never the obstacle (an earlier
note here that said it "needs Python 3.13+" was wrong).
