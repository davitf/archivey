...
| A5 | `listing_limits.py:24` | `extra: dict[str, Any]` | **done** — `dict[str, object]` with Q1; **`Mapping[str, object]` in staged PR 8** (the function only iterates `.values()`, so a read-only view is the right parameter type) |
