...
| A5 | `listing_limits.py:24` | `extra: dict[str, Any]` | **done** — `dict[str, object]` with Q1; **`Mapping[str, object]` in staged PR 8** (a `dict` subclass is a Mapping, a TypedDict was not) |
...
| ID | Site | Where | Notes |
|---|---|---|---|
| A26 | `types.py:451` | `ArchiveMember.extra` | **done** — `dict[str, object]` in the inventory PR; **`MemberExtra` (`dict[str, object]` subclass, overloaded `__getitem__`) in staged PR 8** |
| A27 | `types.py:570` | `ArchiveInfo.extra` | **done** — `dict[str, object]` in the inventory PR; **`ArchiveInfoExtra` (same shape) in staged PR 8** |
