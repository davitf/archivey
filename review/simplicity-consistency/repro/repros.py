...
def r2_tar_corrupt_final_header() -> None:
    """RA vs streaming honesty for a corrupt trailing TAR block (seed A).

    A naive trailing ``X*512`` may still soft-succeed depending on tarfile/EOF
    probe behaviour — record whatever ships today.
    """
    print("\n[R2] TAR corrupt final header — RA vs streaming honesty")

    # Valid tar + one extra non-null block posing as a final header.
    tbuf = io.BytesIO()
    with tarfile.open(fileobj=tbuf, mode="w") as t:
        info = tarfile.TarInfo("a.txt")
        payload = b"hello"
    ...
    try:
        with open_archive(io.BytesIO(data)) as r:
            ...
            diags = r.diagnostics
        print(
            f"  RA: OK names={names} total_diags={diags.total_count} "
            f"codes={[c.value for c in diags.counts]}"
        )
    except CorruptionError as e:
        print(f"  RA: CorruptionError (expected by older seed) — {e}")
    ...
    try:
        ...
        if DiagnosticCode.ARCHIVE_EOF_MARKER_MISSING in diags.counts:
            print("  streaming: ARCHIVE_EOF_MARKER_MISSING present")
    except CorruptionError as e:
        print(f"  streaming: CorruptionError (would be parity) — {e}")
    except ArchiveyError as e:
            ...
            try:
                with open_archive(p, password="WRONG") as r:
                    ...
                    try:
                        ...
                        diags = [d.code for d in r.diagnostics()]
