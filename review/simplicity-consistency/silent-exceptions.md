...
## F3 — ZIP underlying "already closed" → CorruptionError (CONFIRMED)

ZIP maps **all** `ValueError` to `CorruptionError` (`zip_reader.py` translator
ValueError arm):
...
Raise sites include `zip_reader.py` messages
`"Attempt to use ZIP archive that was already closed"`.

`ArchiveStream._fail` only special-cases `"closed file"` in the message —
**"already closed" does not match**.

**Repro (`repros.py` R10 / guardrail):**

```python
with open_archive(io.BytesIO(zip_bytes)) as r:
    member = next(m for m in r.members() if m.type.name == "FILE")
    r._archive.close()   # underlying ZipFile.fp = None; reader still "open"
    r.open(member)       # CorruptionError (misclassified)
```

**Not this finding:** normal `reader.close()` then `open`/`members` already
raises `ArchiveyUsageError` (`#225`). Only the **underlying ZipFile closed while
the reader is still live** path is wrong.
