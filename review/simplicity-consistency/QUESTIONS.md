...
- **Vehicle:** bug fix PR + promote `tests/test_guardrails.py` F1 red → green.
...
| Option | Effect |
|---|---|
| A — Reject (password-parallel) | Loud; may surprise callers who pass encoding everywhere |
| B — Document ZIP/TAR-only | Status quo + honesty |
| C — One diagnostic | Soft middle |

- **Recommend:** A for static `encoding=` when backend would ignore; allow `None`.
  Mirror password gate with `SUPPORTS_ENCODING` or a central allowlist (ZIP, TAR).
- **Pay before tag?** Yes if choosing A; else B is docs-only.

## Q3 — ZIP underlying already-closed → CorruptionError (F3)

Normal `reader.close()` then `open` is already `ArchiveyUsageError`. The bug is
closing the underlying `ZipFile` while the reader is live (blanket
ValueError→Corruption map).

- **Recommend:** carve "already closed" → `ArchiveyUsageError`; consider narrowing
  the blanket ValueError→Corruption arm to known corruption substrings.
- **Vehicle:** bug fix PR.
- **Pay before tag?** Yes.
...
Also still open from O-23: plain `warnings.warn` on solid out-of-order `open()`?
(Spec says silent; no call in `src/` today.)

- **Recommend:** Keep STREAM_REWIND for `0.2.0` with an explicit taxonomy note;
  do **not** add a solid-open warning (already decided silent).
...
- **Recommend:** pick one spelling before tag, or explicitly accept dual with a
  one-line guide note. Rename is free now.
- **Pay before tag?** Freeze-cost argument for deciding.

## Q8 — CLI vs library defaults (F9)

Already decided in `cli-product` Q1. Reconfirm: **accept** as product law;
must-explain #23 + cli.md must stay accurate. No re-litigation.
...
TAR streaming final-header (P3 format law — already tested).
