...
| A-34 | wrong | Spec `format-detection` SFX requirement matches the prose, but shipped `detect_format` has **no SFX scan** (module comment: SFX arrives “in later stages”; no `sfx_scan` path). Repro: `MZ` + real RAR/7z payload → `FormatDetectionError` on detect/auto-open; `format=RAR` works via parser SFX scan. **Prose (and spec) ahead of `detect_format` code.** |
