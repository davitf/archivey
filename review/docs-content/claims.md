...
- **`ZST` reports `FULL` with no `missing` entry on Python 3.11** — this leg has no
  `zstandard` and no `backports.zstd` installed under that name, yet zstd is available.
  Which package satisfies it, and whether `install.md`/`formats.md`'s "`[recommended]` →
  `backports.zstd`" row (`formats.md:18`) describes this correctly, is row **G-6**.
...
| C-50 | Members with `is_current=False` stay visible in listings but are **skipped on extract by default** | `extracting.md:166-167`, `opening-and-listing.md:176-177`, `formats.md:120-122` | `safe-extraction:254` | Keep | |
...
| F-22 | **`STREAM_REWIND_REDECOMPRESSES` fires on cost, not on codec name** — when the rewind discards more than about a megabyte of decoded progress | `access-and-cost.md:100-102`, `gotchas.md:17-18` | `seekable-decompressor-streams:182`, `src/archivey/diagnostics.py:78`, `:281` | Keep, tighten | |
