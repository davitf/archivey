...
| `sweep/` | The whole-codebase **sweep** — a cold end-to-end reading pass over `src/`, one batch at a time | Conventions only; the state lives on [#315](https://github.com/davitf/archivey/pull/315). Holds the batch [`brief-template.md`](sweep/brief-template.md) and the `SWEPT` per-file marker rule that makes coverage countable. 23% of `src/` read as of 2026-09-19; S1 and S2 done, S3–S14 waiting. |
