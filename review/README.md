...
| `typing-escape-hatches/` | Type-checker escape hatches in `src/` — `# type: ignore`, `cast()`, `Any`, `TypeGuard` | Brief 2026-09-11 at `8e88e4f`. **Inventory 2026-09-17** at `94468bd0` (`SUMMARY.md` + `inventory.md`). 89 sites dispositioned; S1/S6 closed; staged fix PRs listed in the SUMMARY. Parallel with `exception-catchalls/` (disjoint sources). |
| `sweep/` | The whole-codebase **sweep** — a cold end-to-end reading pass over `src/`, one batch at a time | Conventions only; the state lives on [#315](https://github.com/davitf/archivey/pull/315). Holds the batch [`brief-template.md`](sweep/brief-template.md) and the `SWEPT` per-file marker rule that makes coverage countable. Batches S0 to S2, S15 and S16 are done; how much of `src/` that is comes from `scripts/sweep_coverage.py`, not from a figure written here. |
| [`api-freeze/`](api-freeze/brief.md) | The **public API at the `0.2.0` freeze** — a re-review of `archive/2026-07-19-api-coherence/` | Brief 2026-09-21 at `b0fe664`. Commissioned because the surface turned over without the count moving: `__all__` went 90 → 89, but 15 names left and **14 arrived**, and the nine public modules are +1562/−293 lines against the July baseline. Deliberately **non-security** — threat analysis hands off to `dev-docs/threat-model.md`. |
| `exception-catchalls/` | Blind `except Exception` / `except BaseException` in `src/` | Brief 2026-09-11 at `8e88e4f`. Verify each blind handler either re-raises or records why swallowing is correct; tighten the rest. A **verification** review — recon found every marked site already carries an inline reason, in five recognisable patterns. Sibling of `typing-escape-hatches/`. **Done 2026-09-25** at `5bbbfdc` ([`SUMMARY.md`](exception-catchalls/SUMMARY.md)): 67 handlers, six fixed, five deferred behind two open PRs; house rules in `dev-docs/topics/exception-handlers.md`. |
