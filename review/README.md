...
| [`api-freeze/`](api-freeze/brief.md) | The **public API at the `0.2.0` freeze** — a re-review of `archive/2026-07-19-api-coherence/` | Brief 2026-09-21 at `b0fe664`, refreshed 2026-09-25 at `c599fc5`. Commissioned because the surface turned over without the count moving: `__all__` is 90 as in July, but 15 names left and **15 arrived**, and the public modules are +3271/−403 lines against the July baseline. Runs once `#465`, `#468` and `#470` have landed. Diagnostics first (§0): the maintainer's own question is whether a new user can tell from the docs when to look at them and what to do. Deliberately **non-security** — threat analysis hands off to `dev-docs/threat-model.md`. |
