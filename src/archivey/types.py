...
# Final keeps a MemberExtra subscript with this constant a literal key; without
# it a checker that widens the assignment to str rejects the write.
...
class MemberExtra(dict[str, object]):
    """Open bag of format-specific fields on :class:`ArchiveMember`.

    A ``dict[str, object]`` subclass. A subscript of a known key narrows to that
    key's declared type; unknown keys stay legal and read as ``object``. Writes
    are not type-checked — the ``str`` fallback is what unknown keys need. The
    ``EXTRA_*`` constants on this module are the names for the keys they cover.

    Known keys: ``is_junction`` (bool); ``rar.created_is_ctime`` (bool),
    ``rar.extract_version`` / ``rar.file_version`` / ``rar.tweaked_crc32``
    (int), ``rar.tweaked_blake2sp`` (bytes); ``zip.compress_type`` /
    ``zip.aes_vendor_version`` / ``zip.aes_strength`` / ``zip.aes_actual_method``
    (int); ``tar.type`` (bytes), ``tar.pax_headers`` (dict[str, str]),
    ``tar.devmajor`` / ``tar.devminor`` (int); ``gzip.original_filename`` (str).
    """
...
class ArchiveInfoExtra(dict[str, object]):
    """Open bag of format-specific fields on :class:`ArchiveInfo`.

    Same shape as :class:`MemberExtra`, over a separate key set. Known keys:
    ``iso.namespace`` (str); ``zip.volume_count`` / ``rar.volume_count`` /
    ``7z.volume_count`` (int).
    """
...
class ArchiveMember:
    ...
    Known keys narrow on a subscript read; unknown keys (third-party or future)
    stay legal and read as ``object``. See :class:`MemberExtra`. The ``EXTRA_*``
...
class ArchiveInfo:
    ...
    narrow on a subscript read; unknown keys stay legal and read as ``object``.
    See :class:`ArchiveInfoExtra`."""
