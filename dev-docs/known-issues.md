...
so it stays in-process. Where no child can be started (a frozen app), a member past the
limit raises `ResourceLimitError`; `None` holds any member in-process. Measured:
