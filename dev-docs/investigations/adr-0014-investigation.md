# ADR 0014 — investigation, trade-offs, and implementation notes

Split out of `dev-docs/decisions/0014-integrity-verdicts-from-reads-not-close.md`
(review/docs `DECISIONS.md` D5). That ADR was 615 lines — 59% of the whole ADR
corpus and roughly 25× the median — and carried an `## Open questions` section while
marked `Status: accepted`. The record itself is now ADR-shaped; everything that made
it long is here, unedited.

The user-facing half — the guarantee and its call × failure matrix — is published on
`docs/reading-members.md`, which is the only copy a caller needs.
...
- **Seek-state tracking.** Once a seek off the sequential frontier disables the
  ...
  above; the hashed-frontier generalization is in `dev-docs/IDEAS.md`). Length / over-run / truncation
