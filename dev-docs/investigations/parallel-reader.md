...
| **ZIP** | RA, independent | **Yes** | `_raw_member_stream` → a `SharedView` over the member's payload, with its own position and the reader's handle lock around each read; the decrypt stage and codec are fresh per open. No per-open scratch on `ZipReader`. |
