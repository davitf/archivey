...
seek that moves the position gives up the HMAC, as it gives up the CRC.
...
| WinZip AES seeks by restarting CTR at the target's block, and gives up the HMAC | CTR decrypts from any offset (counter `1 + offset // 16`), so a seek costs one block. The HMAC covers the whole ciphertext in order: keeping it would mean decrypting everything a seek skips, or a second pass, for a check a seek already forfeits on CRC members (ADR 0014) | Refusing to seek, which broke the `seekable_members=True` guarantee; verifying the HMAC in a second pass on close |
| Accelerators stay off over the WinZip AES stage, even under `ON` | rapidgzip reads its input out of order, so over a seekable AES stage its own seeks would void the HMAC of a member the caller reads straight through. On AE-2 the HMAC is the only check, and with several candidates it is the confirmation | Honouring `ON` there, which trades the tamper check for decode speed without telling the caller |
...
| WinZip AES seeks at every block phase and backward; a moving seek forfeits the HMAC, a seek to the current position keeps it | `tests/test_zip_aes.py::test_aes_decrypt_stream_seeks_at_every_block_phase`, `::test_aes_decrypt_stream_seek_forfeits_the_hmac`, `::test_aes_decrypt_stream_seek_to_current_position_keeps_the_hmac`, `::test_aes_member_seeks`, `tests/test_member_stream_contract.py` (`zip-aes` rows) |
| Accelerators stay off over the WinZip AES stage under `AUTO` and `ON`, with one password and with two, so a tampered AE-2 DEFLATE member still raises; `ON` does not ask for a missing rapidgzip there | `tests/test_zip_aes.py::test_aes_hmac_survives_a_seekable_accelerator`, `::test_aes_member_does_not_ask_for_a_missing_accelerator` |
