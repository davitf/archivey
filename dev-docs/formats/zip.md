...
| `modified` / `accessed` / `created` | CDH DOS date-time (naive local, 2-second granularity) ← NTFS extra `0x000A` (UTC) ← Extended Timestamp `0x5455` (UTC), later overriding earlier — parsed by archivey; `zipfile` only surfaces the DOS field and the raw `extra`. `created` comes from the NTFS field only, a Windows birth time. The Extended Timestamp's third time is what Info-ZIP on Unix fills from `st_ctime`: it goes to `extra["zip.ctime"]` and never overrides `created` | 1980 sentinel, or every layer invalid — with `MEMBER_TIMESTAMP_INVALID` |
...
| `extra` | `zip.compress_type`, plus `zip.ctime` when an Extended Timestamp carries its third time, plus `zip.aes_vendor_version` / `zip.aes_strength` / `zip.aes_actual_method` on AE members, plus `is_reparse_point` from the attribute bit and `is_junction` when a stored reparse buffer says so (§2.2.1) | — |
