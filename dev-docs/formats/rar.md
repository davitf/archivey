...
| Refuse a glob member name whose mask also matches earlier members; a config flag is the escape hatch | The read is correct — the skip returns the member's own bytes — but `unrar` has already decompressed everything ahead of it, and nothing bounds that: `ExtractionLimits` stop at extraction and `AccessCost.DIRECT` says the opposite on a nonsolid archive. Measured on this tree, where the mask still carries the stored `*`: a 120 KB member named `*.bin` costs 2,000,000 bytes of `data.bin` first; a member named `*` costs the archive. [#371](https://github.com/davitf/archivey/pull/371), open at the time of writing, would substitute `?` for every `*` in the mask, after which a nonzero prefix needs a **same-length** sibling agreeing on every literal position — that narrows the set sharply without bounding the worst case, since one same-length sibling can be any size, so it does not make this refusal redundant. Maintainer (davitf, 2026-09-19): "archives with filenames like these are likely malicious, not worth the DOS risk. config is an escape hatch" | A diagnostic instead of a refusal, which reports a cost already paid; refusing with no flag, which would reject a real `report*.pdf` beside `report1.pdf`; a byte budget, which is a limit on work already done by the time it trips. Note what this does **not** cover: a name with no sibling it can match has a zero prefix, is not refused, and still reaches `unrar` as a mask. Mask-matching cost is a separate narrowing, which is what #371 is for; neither fix subsumes the other |
