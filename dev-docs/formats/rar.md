...
| Match the demux mask with a two-pointer walk, not a compiled regex | Spelling `*` as `.*` backtracks exponentially on a mask that alternates wildcards with literals the subject can satisfy many ways, and **both operands come from the archive** — the member name is the mask and a sibling is the subject, so a hostile name picks them. Measured: `"a" + "*a"*n + "b.txt"` against `"a"*60 + ".txt"` cost 0.039 s at n=5 and 18.2 s at n=8, about 8x per added `*`, in a name with room for hundreds; a 271-byte two-member archive spent 32.9 s inside `reader.open()` **before `unrar` was spawned**, so no subprocess timeout applied. The two-pointer walk is O(len(name) x len(mask)) and measured 0.000016 s on the 18.2 s case, flat at 8, 64 and 256 wildcards. Equivalence to the regex grammar is pinned exhaustively over an alphabet that includes `*` and `?` **on the name side** — the wildcard arm has to be tested before character equality, or a mask `*` matches a literal `*` in the name and under-matches | Keeping the regex; bounding the wildcard count, which also fixes `unrar`'s own half (below) but refuses archives that read today |
| `unrar`'s own mask matcher has the same defect, and we still hand it the mask | Not fixed, and not fixable from here: `unrar p -inul -cfg- -p- '-n./a*a*a*a*a*a*a*a*ab.txt'` alone takes 15.46 s against 0.007 s for a plain mask on the same archive, so the 271-byte case above still costs 15.6 s end to end after archivey's half was fixed. Bounding the wildcard count before handing the mask over is the only lever we have, and it changes which archives are readable | Silently capping it; assuming a subprocess timeout covers it, when the cost is inside a process we wait on |
...
Windows `Rar.exe` banner unmeasured). **#8** amortize solid random reads via
