...
| Refuses | Non-seekable sources · a non-RARLAB `unrar`/`rar` (no fallback to `unar` / `7z` / `bsdtar` / `unrar-free`) · a RARLAB binary older than 6.0, or one whose banner version cannot be parsed · a later volume opened without its first · a glob in a directory component, or a backslash in the stored name (unrar path) · writing |
...
exist. Reads still spawn only `p` (§3, §10 #20).
...
- **`unrar` seeks the archive, so it cannot be piped one** — which is why a stream source
  ...
  archive. `CostReceipt.notes` states that caveat at open (§10 #6 layer 1). There is no
...
non-path stream, `CostReceipt.notes` states that caveat **at open** (§10 #6 layer 1) —
...
preferring `unrar`, still spawning only `p` (§10 #20). Windows `Rar.exe` banner is
...
| Opening a glob-named member whose mask also matches **earlier** members raises `UnsupportedFeatureError` | **archivey** | `unrar -n./a*.txt` decompresses every match and concatenates them, so the read costs the earlier ones too — unbounded, and outside `ExtractionLimits`, which do not reach `open()`/`read()`. Refused by default; `ArchiveyConfig.rar_allow_glob_member_concatenation=True` reads it anyway, and the error names the flag. A glob name matching nothing else is unaffected. A solid `stream_members()` pass builds no mask, so it is unaffected too — a **nonsolid** one takes the named route and is refused (§2.3, §6) |
...
| Refuse a glob member name whose mask also matches earlier members; a config flag is the escape hatch | The read is correct — the skip returns the member's own bytes — but `unrar` has already decompressed everything ahead of it, and nothing bounds that: `ExtractionLimits` stop at extraction and `AccessCost.DIRECT` says the opposite on a nonsolid archive. Measured: a 120 KB member named `*.bin` costs 2,000,000 bytes of `data.bin` first; a member named `*` costs the archive. After #371's `?` substitution a nonzero prefix needs a **same-length** sibling agreeing on every literal position, which narrows the set sharply without bounding the worst case — one same-length sibling can be any size. Maintainer (davitf, 2026-09-19): "archives with filenames like these are likely malicious, not worth the DOS risk. config is an escape hatch" | A diagnostic instead of a refusal, which reports a cost already paid; refusing with no flag, which would reject a real `report*.pdf` beside `report1.pdf`; a byte budget, which is a limit on work already done by the time it trips. Note what this does **not** cover: a name with no sibling it can match has a zero prefix, is not refused, and still reaches `unrar` as a mask — mask-matching cost is bounded separately, by the `?` substitution, and neither fix subsumes the other |
...
- **Can the stream-source copy be made small, rather than just moved?** Layer 1 of P11
  ...
  rather than relocating the whole archive (§10 #6 layer 2). The sibling cost — an
...
volume 1 of that set; **#7** `unrar` version floor **6.0**, parsed from the
identification banner and cached with the probe; and **#20** RARLAB `rar` accepted
when `unrar` is missing (`unrar` preferred; banner `RAR x.yy`; still `p` only;
Windows `Rar.exe` banner unmeasured); **#21** stream volumes materialize from
`_ensure_archive_path()` rather than the constructor, so listing a stream-volume set
writes nothing (§2.3, §6); and **#19** a glob name whose mask also matches earlier
members is refused, with `rar_allow_glob_member_concatenation` as the escape hatch
(§5, §6). **#8** amortize solid random reads via
