...
| `extra` | `rar.file_version` on a history row; `rar.tweaked_crc32` / `rar.tweaked_blake2sp` on a tweaked-digest member; `rar.created_is_ctime` when `created` is present (see that row); `rar.extract_version` when the FILE header recorded one (stored and compressed) | — |
...
| M0 is `STORED`; M1–M5 is `RAR` with `level` 1–5; unpack version in `extra["rar.extract_version"]` (stored included); method bytes outside M0–M5 stay `UNKNOWN` with no `level` | `tests/test_rar_reader.py::test_member_reports_exact_compression_and_extract_version`, `::test_unknown_method_byte_omits_level`, `::test_stored_m0_direct_read`, `tests/test_rar_oracle.py::test_native_rar_matches_rarfile_metadata_and_bytes` |
...
**#4** solid link emission per generation ([#301](https://github.com/davitf/archivey/pull/301)); **#9** compressed members report `CompressionAlgorithm.RAR` with M1–M5 in `level` and extract version in `extra["rar.extract_version"]` ([#308](https://github.com/davitf/archivey/pull/308)); **#10** RAR5/RAR3 `accessed`/`created` from the time extra ([#300](https://github.com/davitf/archivey/pull/300)); **#13** `close()` chaining and **#14**
