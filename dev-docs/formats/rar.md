...
| An encrypted RAR 1.5 / 2.x old-style comment (PASSWORD or SALT flag) is `None`, with or without `unrar` or a password | **archivey** | Known limitation, because it cannot be tested: RARLAB `rar` 7.00 writes no old-style comment subblocks and no repo fixture carries either flag. The comment subblock keeps no salt, so the synthetic FILE header `decompress_rar3_blob` builds could not carry one; it returns `None` before spawning anything. It also no longer copies the comment's DICTMASK bits, which do not declare a dictionary on a comment subblock (§2.2) |
...
| An encrypted old-style comment is `None` and spawns nothing | `::test_rar3_encrypted_old_style_comment_is_skipped_up_front` |
