...
| Opening a glob-named member whose mask also matches **earlier** members raises `UnsupportedFeatureError` | **archivey** | Names like this are almost always constructed. On a nonsolid archive `unrar -n./a*.txt` also decompresses every earlier match — unbounded, and outside `ExtractionLimits`, which do not reach `open()`/`read()`. On a solid archive that decode is already inside `AccessCost.SOLID`, and what the mask adds is only the transfer of bytes archivey then discards — measured at roughly 1 ms per MB, a bounded factor on work the read already owes (§6). Refused either way; `ArchiveyConfig.rar_allow_glob_member_concatenation=True` reads it anyway, and the error names the flag. A glob name matching nothing else is unaffected. A solid `stream_members()` pass builds no mask, so it is unaffected — a **nonsolid** one takes the named route and is refused like any other mode, which is deliberate (§2.3, §6) |
...
| Refuse a glob member name whose mask also matches earlier members; a config flag is the escape hatch | Names like this are almost always constructed. The read is correct — the skip returns the member's own bytes — but on a nonsolid archive `unrar` has already decompressed everything ahead of it, and nothing bounds that: `ExtractionLimits` stop at extraction. On a solid archive those matching members are already inside the solid prefix `AccessCost.SOLID` advertises, so what the mask adds there is not the decode but only the *emit*; they are refused anyway. Measured on this tree, where the mask still carries the stored `*`: a 120 KB member named `*.bin` costs 2,000,000 bytes of `data.bin` first on a nonsolid archive; a member named `*` costs the archive. [#371](https://github.com/davitf/archivey/pull/371), open at the time of writing, would substitute `?` for every `*` in the mask, after which a nonzero prefix needs a **same-length** sibling agreeing on every literal position — that narrows the set sharply without bounding the worst case, since one same-length sibling can be any size, so it does not make this refusal redundant. Maintainer (davitf, 2026-09-19): "archives with filenames like these are likely malicious, not worth the DOS risk. config is an escape hatch". What the solid *emit* actually costs was then measured (2026-09-20, unrar 7.00, `p -inul -n./`, two members, the first unpacking to 66 MB, output read and dropped, median of 15): the no-glob read costs 0.035 s when that first member is highly compressible and 0.380 s when it is incompressible, and the glob mask adds 0.074 s and 0.052 s on top — a transfer cost of roughly 1 ms/MB, so 3.1× the read in the first case and 1.14× in the second, against **23×** for the same pair nonsolid, where none of that decode would otherwise happen at all. So the refusal does buy something on a solid archive, but a bounded factor on work already owed rather than new work. Maintainer (davi, 2026-09-20), asked whether to carve solid out: "I lean towards always rejecting, for consistency internally and to keep the meaning of the config option" — and in the same breath flagged the whole refusal as one to revisit, which is §7 | A diagnostic instead of a refusal, which reports a cost already paid; refusing with no flag, which would reject a real `report*.pdf` beside `report1.pdf`; a byte budget, which is a limit on work already done by the time it trips. Note what this does **not** cover: a name with no sibling it can match has a zero prefix, is not refused, and still reaches `unrar` as a mask. Mask-matching cost is a separate narrowing, which is what #371 is for; neither fix subsumes the other |
...
- **Does the glob-concatenation refusal earn its keep?** It ships and is decided (§6):
  ...
  default, on solid and nonsolid archives alike. The maintainer flagged the ruling itself
  as one to revisit (davi, 2026-09-20: "this whole rejection thing is a decision we might
  revisit later"), on two arguments that the measurement in §6 does not dispose of.

  First, **an attacker can choose the archive shape.** The sharp case — decode that would
  not otherwise happen, unbounded in the earlier member's size, measured at 23× — is the
  nonsolid one. Make the same archive solid and the amplification the refusal prevents
  drops to a bounded transfer cost, 1.14–3.1× of a read the caller already owes. A
  hostile archive is cheap to rebuild, so the refusal raises the cost of one construction
  rather than closing the amplification.
