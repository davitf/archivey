...
| ~~`backends/zip_aes.py`~~ | 0 | 15 | **Done** — batch H: the AES-CTR keystream is a `CryptoBackend` stage with the counter convention as a parameter, written block-wise (Z-K1); `WinZipAesDecryptStream` stays as the ZIP AE boundary. 14 (placement) was closed by the move in [#443](https://github.com/davitf/archivey/pull/443) |
...
**Thread 15 is done (batch H).** `zip_aes.py` imported `cryptography` directly, under a
...
| **F — placement + odds** | `backends/zip_aes.py`, `volumes.py`, `reader_state.py` | 11, 12 | **Placement done** (threads 10/14, ruled 2026-09-19; the five modules moved under `backends/` in [#443](https://github.com/davitf/archivey/pull/443)). Thread 15 was closed by batch H. Two threads left. 51, 53 and 54 were closed by [#440](https://github.com/davitf/archivey/pull/440), 13 by #374, and thread 3 left when #342 answered it. |
