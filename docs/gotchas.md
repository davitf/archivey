...
- **Don't expect `seek()` on a WinZip AES member yet.** Encrypted 7z members
  seek when `seekable_members=True` (CBC can restart at a block). WinZip AES
  still raises: HMAC covers the whole ciphertext.
  → [Seeking](access-and-cost.md#seeking-inside-compressed-members)
...
- **A RAR password cannot contain a line break.** `unrar` reads the password as a
  ...
  instead: reading an encrypted RAR member with such a password raises
  `UnsupportedOperationError`. Watch for a password read from a file, which usually
  carries a trailing newline; strip it. Archives with nothing encrypted are unaffected,
  since no password is handed to `unrar` at all. → [RAR](formats.md#rar)
