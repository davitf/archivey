...
| 7z | yes (common codecs) | `[recommended]` for PPMd/Deflate64/zstd/brotli/AES | indexed | solid folders | Native reader; BCJ2 forward-only, pure Python |
...
- **BCJ2** (what 7-Zip writes for x86 executables at `-mx9`) reads on a core install,
  encrypted or not. Its decoder is pure Python and forward-only: about half the speed of
  the same file under BCJ, and a BCJ2 member cannot seek.
