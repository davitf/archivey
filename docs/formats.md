...
- **Brotli** has no magic, so detection uses a content probe plus a framing check:
  a first meta-block that *declares* more bytes than the source holds is rejected.
  Probe-only confidence is `PROBABLE` when the first meta-block is compressed (or when
  the name ends in `.br`), and `GUESS` for uncompressed/metadata-first without that
  extension. A later decode failure on a `GUESS` result sets
