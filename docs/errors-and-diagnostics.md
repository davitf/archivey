...
| `PROBE_FORMAT_UNCONFIRMED` | A single-file format came from a content probe with nothing corroborating it (no matching extension, no inner-TAR upgrade), and a later decode failed — at any detection confidence. The matching exception also has `format_unconfirmed=True`. Partial output may already have been produced. |
