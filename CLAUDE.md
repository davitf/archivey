...
- **Skills for the review loop:** `/code-review-skill` produces findings,
  `/address-review-findings` dispositions them. If you are reacting to PR events on
  autopilot rather than being asked, `.claude/skills/steward/SKILL.md` is the entry point —
  it routes to `address-review-findings` and carries this repo's autonomy boundary. Ask for them by those names — a bare
