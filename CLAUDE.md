...
- **Skills for the review loop:** `/code-review-skill` produces findings,
  ...
  §Review workflow has the handoff, including why agent-authored PR comments must be
  identifiable as such (in a Claude Code session, the
  `_Generated by [Claude Code](https://claude.ai/code)_` footer).
