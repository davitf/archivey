...
- **Skills for the review loop:** `/code-review-skill` produces findings,
  ...
  `/code-review` is a *builtin* skill, not this repo's. The autopilot entry point is a
  **path, not a slash command**: if you are reacting to PR events rather than being asked,
  read `.claude/skills/steward/SKILL.md`, which routes to `address-review-findings` and
  carries this repo's autonomy boundary. `AGENTS.md` §Review workflow has
