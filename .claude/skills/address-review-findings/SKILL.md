...
- **Agent-authored** — carries a `_Generated by [Claude Code](https://claude.ai/code)_`
  footer, or is from a bot login (`cursor[bot]`, `qodo-code-review[bot]`).
...
pushing after `ruff` alone:

```bash
uv run --no-sync ruff format src/ tests/ scripts/ benchmarks/
uv run --no-sync ruff check --fix src/ tests/ scripts/ benchmarks/
uv run --no-sync pyrefly check      # ← both type checkers, not just one
uv run --no-sync ty check
uv run --no-sync pytest
```

Then the three dependency configs from `CONTRIBUTING.md` §"Before pushing…" when behaviour
depends on extras or versions, plus `openspec validate --all`,
`scripts/check_docs_nav.py`, and `scripts/check_openspec_archived.py`.
