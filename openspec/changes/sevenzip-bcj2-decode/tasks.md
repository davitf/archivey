...
- [ ] 1.5 Failing tests: a folder whose coder output is bound twice, or whose bind pairs
...
- [ ] 1.5a Failing test: a forced BCJ2 folder whose `main` branch is `[7zAES, BZip2]` (or
      `-m1=BZip2` unencrypted) decodes; its BZip2 stage gets the branch predecessor's
      size (D1 step 3).
- [ ] 1.5b Failing test: `member.compression` for the `-mx9` fixture is exactly
      `(BCJ2, LZMA2)` (D9).
- [ ] 1.5c Failing test: with `main` followed by a large extra tail, the end-of-output
...
- [ ] 2.1 Move `prototype/bcj2.py` to `src/archivey/internal/streams/bcj2.py`, with its
...
- [ ] 2.2 `plan_folder`: resolve the tree (D1), planning each branch from its own coder
...
- [ ] 2.6 `_build_folder_compression`: for a tree, list the root and its `main` branch
...
- [ ] 3.2 Drop "BCJ2 unsupported" from the same list as the proposal's: `docs/formats.md`,
...
- [ ] 3.3 `dev-docs/threat-model.md`: two rows for D6. One is the per-output-byte CPU
...
- [ ] 3.4 In `dev-docs/IDEAS.md`, on the entry "Threat-model row for archive-declared
