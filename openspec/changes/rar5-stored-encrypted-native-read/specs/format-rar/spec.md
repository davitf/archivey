...
copy every volume to a temp directory. The note is a
...
| Single non-path stream, at open | `ar.cost.notes` warns a spawned read will copy to disk; nothing is written yet |
| Ordered stream volumes, at open | `ar.cost.notes` warns a spawned read will copy every volume; nothing is written yet |
