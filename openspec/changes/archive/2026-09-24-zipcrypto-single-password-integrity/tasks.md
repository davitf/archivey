...
- [x] 1.1 Wrap the single-password lazy ZipCrypto stream so a candidate integrity failure
      on `read`, `readinto` or `seek` raises `EncryptionError` naming both causes.
...
- [x] 2.1 A single password that collides on the check byte raises that `EncryptionError`
      for every compression method, on `read`, `readinto` and a forward `seek`,
      without a wrong-password mark; the right password still reads.
