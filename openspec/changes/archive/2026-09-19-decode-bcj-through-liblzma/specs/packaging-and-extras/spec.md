# packaging-and-extras — drop pybcj delta

> BCJ branch filters decode through liblzma in core now (see the `format-7z` delta), so
> no `src/` module imports `bcj`. The "extras map only to libraries the code uses"
> requirement then forces `pybcj` out of `[recommended]`; nothing else about the extras
> changes.
