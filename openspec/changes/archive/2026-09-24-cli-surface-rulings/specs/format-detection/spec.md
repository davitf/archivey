...
format `open_archive` reads it as. It SHALL NOT raise `IsADirectoryError` or any
other `OSError`.
...
| Directory path | `format=DIRECTORY`, `confidence=CERTAIN`, `detected_by="directory"`; nothing read; no `OSError` |
