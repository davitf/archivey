...
**Decided:** Explicit start offset plus a forced-format scan for parity with RAR. Spec the obligation in format-seven-z so it lives next to other seven-zip contracts. Fast path when magic is already at the open origin stays unchanged. Land with or just before sfx-format-detection.
