...
to a temporary file solely to strip a stub. The scan bound SHALL be at least as
large as the RAR parser’s SFX window so forced-format behaviour stays symmetric.
...
| Forced `format=SEVEN_Z`, `MZ` stub, magic at N within bound | Scan finds magic; open succeeds |
| Forced `format=SEVEN_Z`, no magic within bound | `CorruptionError` (not a silent empty archive) |
