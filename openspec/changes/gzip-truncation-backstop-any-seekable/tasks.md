...
- [x] 2.2 "Does the accelerator have to read through a view?" — **resolved (FINDINGS.md)**: stream
      ...
      `view(0)` — is the path. (ii) is dead: `gzip-multimember-detect-via-index` was found
...
- [x] 3.1 Spike (FINDINGS.md): truncated **non-raising** bytes do **not** trip Bug 3 (soft EOF,
      ...
      sources.
- [x] 3.2 Trap prototype **validated** in the spike: the shim caught the fault internally, served
      benign EOF, exited cleanly, and preserved the `ValueError` for re-raise. Still to do in
      implementation: the production shim + `_AcceleratorStream` re-raise wiring, and the
      wall-clock-timeout sweep to bound the separate finalizer-race trigger (`known-issues.md`).
...
- [ ] 4.4 Multi-member disambiguation on a non-path source: scan an independent `SharedSource`
      ...
      (`gzip-multimember-detect-via-index`) is infeasible (its FINDINGS), so the scan stays — it
...
- [ ] 6.2 Sync into main `seekable-decompressor-streams` when landing. The would-be conflicting
      sibling `gzip-multimember-detect-via-index` is now blocked/no-op (its FINDINGS) and will
