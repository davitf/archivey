...
- [ ] 0.3 **This change now also MODIFIES `Executable-looking prefixes must not silently become a wrong stream format`**, added to the live spec by the same archive. It has to: that requirement enumerates the cue as `MZ` / `\x7fELF` only, which this change widens to Mach-O and `#!`, and it is where the macOS silent-wrong-answer defect is registered. Two notes for whoever archives:
  - **`brotli-probe-framing-gate` (PR #255) now MODIFIES this requirement too, so the two are no longer archive-order independent.** It first tried the lighter touch — cross-referencing the requirement from its own added framing requirement — and a reviewer showed that does not work: `openspec archive` only rewrites requirements a change marks MODIFIED, so this requirement's matrix would have gone on promising `PROBABLE` for an uncorroborated Brotli hit that #255 moves to `GUESS`. The two changes edit **disjoint parts** of it — #255 the probe-tightening paragraph and the confidence rows, this change the cue enumeration and the Mach-O defect — but OpenSpec replaces the whole requirement, so **whichever archives second must rebuild its MODIFIED block on the other's text**. #255 will almost certainly go first (it is a proposal awaiting an implementation PR, this one is not yet started), so expect to inherit: a narrowed "no *threshold* knobs" prohibition, a residual paragraph carrying three clauses instead of one, and two confidence rows in place of the single `PROBABLE` one
...
- [ ] 0.4 **The implementations are nearly disjoint too, with two touchpoints.** Both edit `_detect_format_body`, but at different steps — this change restructures step 2 (the cue and the scan) and inserts a tier before it, while #255 changes step 3 (the content-probe loop) and the codec probe signature. Neither touches the other's files otherwise: `sfx.py` is this change's alone, `codecs.py` is #255's. What actually overlaps:
  - **One shared helper.** Both need "is this source's end reachable, and where is it?" — #255 to compare a Brotli meta-block's declared length against the source, this change to read the last 65 557 bytes for the EOCD. `detection.py` has no such helper today; `_peek_prefix` dispatches four ways (`Path` / `PeekableStream` / seekable / raw) and stops at the prefix. #255's task 1.4 names it `_source_length`; build the tail read on the same dispatch rather than a second one
  - **Task 2.1a's red-green premise is order-dependent.** It says a Mach-O stub + 7z "currently returns `BROTLI`". True on `main`, but #255's framing gate independently rescues that case (a Mach-O's MLEN comes from its `cputype` — 16 636 918 for arm64 — which no ordinary binary can honour), so after #255 the same fixture raises `FormatDetectionError` instead. The test asserts the *real members*, which fails under both, so it stays red-green either way — but do not re-derive the premise from a run on a branch that already has #255
