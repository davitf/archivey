...
- [ ] 5.6 This change **closes** `brotli-probe-framing-gate` tasks **5.3** (the executable cue is blind on macOS) and **5.4** (the `e_lfanew` bound, now task 2.1b here). Check those boxes there when this lands, so there is one record of the work rather than two
