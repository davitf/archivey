...
- [ ] 1.1 Add bounded SFX magic scan (`MZ` / ELF prefix → RAR / 7z / ZIP `PK\x03\x04` needles) in `detect_format`, setting `payload_offset` and `detected_by`
...
- [ ] 3.3 Varied-stub + RAR/7z/ZIP cases from the SFX matrix (match / no-match / extension fallthrough)
- [ ] 3.4 Bare brotli / non-executable streams unchanged
