...
**What real encoders write, surveyed.** Every liblzma-based producer writes the all-ones *unknown* sentinel — for empty input and for known-size input alike: `xz --format=lzma` and the `lzma` CLI at `-0`…`-9` with and without `-e`, and Python at all twenty preset combinations plus custom `(lc, lp, pb, dict_size)` filters. Sentinel every time. A zero **dictionary** is not writable at all: liblzma refuses any value below `LZMA_DIC_MIN` (4096), so the all-zero header is one no encoder can produce.

Size 0 *is* producible, though not by liblzma: the LZMA SDK's own `lzma_alone` writes the actual uncompressed size, so a 0-byte input yields a real dictionary with size 0. **That costs nothing, measured end-to-end.** Such a stream is not claimed by the content probe with or without this gate — `require_output` declines the empty decode first — and a `.lzma` name still opens it through the extension, one empty member, exactly as before. The gate refuses nothing that was ever detected.
