...
occurrence SHALL raise only once the stream is consistent: a read raises before it
consumes, keeping the bytes it decoded for the next read, and a seek or size query
raises after it finishes, with the position where it left it. A caller that catches
the `DiagnosticRaisedError` SHALL be able to keep reading and get the right bytes. Unsafe corruption SHALL remain a typed
