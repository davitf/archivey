...
SHALL be those of both passes together. Each pass runs under the full budget, so such a
receipt MAY exceed it. `max_far_bytes` is separate from `max_prefix_bytes` because a far fixed-offset
...
overlapping requests in full and `unique_bytes_read` stands in for it. A receipt from a
single detection pass that fails `within_budget` SHALL carry a *budget exhausted* or
*capability unavailable* skip naming the tier that was cut short. Remaining reserved fields
...
| Stub-only `vol.exe` beside `vol.7z.001` | `SEVEN_Z`; the receipt includes the stub pass's SFX scan |
