...
1. **Gather all four channels** — top-level comments, inline review threads, formal
   ...
   Note who actually wrote each comment: agents post through the maintainer's account, so
   a `davitf` comment *without* a `_Generated by [Claude Code]_` footer is the human.
