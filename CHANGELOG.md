...
- `ArchiveyConfig.rar_decompressor` chooses the program that reads RAR member data:
  `RarDecompressor.AUTO`, the default (RARLAB `unrar` or `rar` when it is installed,
  `unar` otherwise, chosen once when the archive opens), `RarDecompressor.UNRAR` (RARLAB
  only, the behaviour before this release) or `RarDecompressor.UNAR` (`unar` 1.10 or
  later, which `brew install unar` provides on macOS). A machine with `unar` and no
  RARLAB program now reads RAR member data instead of raising
  `PackageNotInstalledError`. `unar` reads less than `unrar`: encrypted RAR
