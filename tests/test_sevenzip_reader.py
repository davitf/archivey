...
from archivey.internal.password_confirm import CONFIRM_CHUNK_BYTES
...
def test_password_confirm_does_not_request_the_whole_folder(
    ...
    assert spies[0].max_requested <= CONFIRM_CHUNK_BYTES, (
        f"confirm requested {spies[0].max_requested} bytes in one read "
        f"(chunk is {CONFIRM_CHUNK_BYTES}, folder is {folder_size})"
    )
