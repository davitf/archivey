...
def test_no_anchor_small_unit_reads_the_unit() -> None:
    plan = plan_confirm([(10, None)], None, budget=BUDGET, codec_rejects=False)
...
def test_a_unit_shorter_than_four_bytes_never_confirms() -> None:
    plan = plan_confirm([(3, _crc(b"abc"))], None, budget=BUDGET, codec_rejects=False)
    assert run_confirm_plan(io.BytesIO(b"abc"), plan) is ConfirmVerdict.INCONCLUSIVE
    assert run_confirm_plan(io.BytesIO(b"abd"), plan) is ConfirmVerdict.REJECTED
...
def test_run_confirms_on_matching_member_crcs() -> None:
    ...
    assert run_confirm_plan(io.BytesIO(first + second), plan) is (
        ConfirmVerdict.CONFIRMED
    )


def test_run_rejects_a_mismatched_member_crc() -> None:
    plan = plan_confirm([(4, 0xDEADBEEF)], None, budget=BUDGET, codec_rejects=False)
    assert run_confirm_plan(io.BytesIO(b"abcd"), plan) is ConfirmVerdict.REJECTED


def test_run_rejects_a_mismatched_folder_digest() -> None:
    plan = plan_confirm([(4, None)], 0xDEADBEEF, budget=BUDGET, codec_rejects=False)
    assert run_confirm_plan(io.BytesIO(b"abcd"), plan) is ConfirmVerdict.REJECTED
...
def test_run_confirms_a_matching_folder_digest() -> None:
    ...
    assert run_confirm_plan(io.BytesIO(payload), plan) is ConfirmVerdict.CONFIRMED


def test_run_rejects_a_short_stream() -> None:
    # The CRC is the one the short bytes hash to, so only the short-read guard can
    # reject here.
    plan = ConfirmPlan(((8, _crc(b"short")),), None, confirms=True, bounded=True)
    assert run_confirm_plan(io.BytesIO(b"short"), plan) is ConfirmVerdict.REJECTED


def test_run_rejects_a_short_stream_with_no_anchor() -> None:
    plan = plan_confirm([(8, None)], None, budget=BUDGET, codec_rejects=False)
...
def test_run_reads_in_bounded_chunks() -> None:
    ...
    assert run_confirm_plan(spy, plan) is ConfirmVerdict.CONFIRMED
    assert spy.max_requested <= CONFIRM_CHUNK_BYTES
...
def test_the_input_cap_is_the_inner_tar_probe_bound() -> None:
    ...
    assert CONFIRM_MAX_INPUT_BYTES == detection._INNER_TAR_MAX_PROBE_BYTES
    # Cut on an AES block boundary, so a capped CBC stream never ends mid-block.
    assert CONFIRM_MAX_INPUT_BYTES % 16 == 0
...
def test_rejecting_codecs_reject_random_input(codec: Codec) -> None:
    ...
    one of these stops at ``CONFIRM_PREFIX_BYTES`` instead of walking to a late CRC.
...
) -> tuple[UnverifiedReadWatch, _Calls]:
    calls = _Calls()
    watch = UnverifiedReadWatch(
        io.BytesIO(data),
        size=len(data),
        on_unverified=calls,
